...
from dealership_logic import get_dealership_response, get_available_bikes, get_service_packages, get_dealerships
from location_service import process_location_query, set_user_location
from human_agent_fallback import should_escalate_to_human, escalate_query, get_agent_response
import random
...
class VoiceAssistant:
    ...
    def __init__(self):
        ...
        self.openai_client = GeminiClient(api_key=os.environ.get('GEMINI_API_KEY'))
    ...
    def get_dealership_response(self, query: str) -> str:
        """
        Get response from dealership logic system.

        Args:
            query (str): The user's query in English

        Returns:
            str: Response from dealership system
        """
        try:
            return get_dealership_response(query)
        except Exception as e:
            print(f"Error in dealership logic: {e}")
            return "I'm having trouble accessing the dealership information right now. Let me help you with general questions instead."

    def conduct_conversation(self, language_code):
        """
        Conducts a conversation with the user in the specified language.

        This method continuously listens for user input, processes it, and generates a response using Google's Gemini AI models.
        ...
        Note:
            ...
            the speech recognizer, translation service, and Gemini AI model. Ensure that
            ...
            with sr.Microphone() as source:
                ...
                try:
                    ...
                    if user_input_english.lower() in stop_phrases:
                        ...
                        # First try to handle with dealership logic
                        dealership_response = self.get_dealership_response(user_input_english)

                        # If dealership logic returns a generic response, use AI for general conversation
                        if "I can help you with" in dealership_response or "Please let me know" in dealership_response:
                            response = self.openai_client.chat_with_gemini(user_input_english)
                        else:
                            response = dealership_response
